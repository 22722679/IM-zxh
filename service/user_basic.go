...
func UserQuery(c *gin.Context) {
	...
	if models.JudgeUserIdFriend(userBasic.Identity, uc.Identity) {
		data.IsFriend = true
	}
	c.JSON(http.StatusOK, gin.H{
		"code": 200,
...
func UserAdd(c *gin.Context) {
	...
	uc := c.MustGet("user_claims").(*helper.UserClaims)
	if models.JudgeUserIdFriend(ub.Identity, uc.Identity) {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "互为好友，不可重复添加",
		})
...
type UserQueryResult struct {
	...
	IsFriend bool   `json:"is_friend"` //是否是好友，true是好友关系，false不是好友关系
}
